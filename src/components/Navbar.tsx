...
export function Navbar() {
  ...
  // Helper untuk mendapatkan teks dengan fallback
    ...
    <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-black/80 backdrop-blur-md py-2' : 'bg-transparent py-4'}`}>
      <div className="max-w-7xl mx-auto px-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          ...
          >
            {getText('nav.about', fallbackTexts.about)}
          </button>
          <button 
            onClick={() => handleScrollToSection("products")}
            className="text-gray-300 hover:text-white transition-colors"
          >
            {getText('nav.products', fallbackTexts.products)}
          </button>
          <button 
            onClick={() => handleScrollToSection("tech-stack")}
            className="text-gray-300 hover:text-white transition-colors"
          >
            {fallbackTexts.techStack}
          </button>
          <button 
            onClick={() => navigateToPage('/mini-games')}
            className="text-gray-300 hover:text-white transition-colors"
          >
            {fallbackTexts.miniGames}
          </button>
          <button 
            onClick={() => handleScrollToSection("testimonials")}
            className="text-gray-300 hover:text-white transition-colors"
          >
            {getText('nav.testimonials', fallbackTexts.testimonials)}
          </button>
          <button 
            onClick={() => handleScrollToSection("contact")}
            className="text-gray-300 hover:text-white transition-colors"
          >
            {getText('nav.contact', fallbackTexts.contact)}
          </button>
        ...
        <div className="flex items-center gap-4">
          <motion.button 
            className="hidden sm:block nav-download px-5 py-2 bg-gradient-to-r from-indigo-500 to-purple-500 text-white font-medium rounded-full hover:opacity-90 transition-all border border-white/30 shadow-lg shadow-purple-500/30"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          >
            {getText('nav.download', fallbackTexts.download)}
          </motion.button>
      ...
      {isMobileMenuOpen && (
        ...
        >
          <div className="flex flex-col gap-4">
            ...
            <motion.button 
              className="w-full px-5 py-2 nav-download bg-gradient-to-r from-indigo-500 to-purple-500 text-white font-medium rounded-full hover:opacity-90 transition-all border border-white/30 shadow-lg shadow-purple-500/30"
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
